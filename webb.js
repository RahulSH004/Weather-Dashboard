const API_KEY_UNSPLASH = 'RqSf2TmE1--DqmTF1WnQNzsfDVOY5FTBqxJ406wxu8w';
...
async function fetchImages(cityName, page = 1) {
    const response = await fetch(`https://api.unsplash.com/search/photos?page=1&query=${cityName}&client_id=${API_KEY_UNSPLASH}&orientation=landscape`);
    const data = await response.json();

    if (response.ok && data.results.length > 0) {
        state.totalPages = data.total_pages;
        const imageUrl = data.results[Math.floor(Math.random() * data.results.length)].urls.full;
...
async function updateBackgroundImages(cityName) {
    try {
        if (state.currentCity === cityName) {
            // Fetch next image from the current city
            state.currentPage = (state.currentPage % state.totalPages) + 1;
        } else {
            // Reset to the first page for a new city
            state.currentCity = cityName;
            state.currentPage = 1;
        }
        const newImageUrl = await fetchImages(cityName, state.currentPage);
        if (newImageUrl) {
            const img = new Image();
            img.src = newImageUrl;
            img.onload = () => {
                ...
                // Remove the class after the transition to reset
                setTimeout(() => {
                    container.classList.remove('transitioning');
                }, 1000); // Matches the duration of the transition in CSS
            };
        } else {
            container.style.backgroundColor = 'black';
            container.innerHTML = '<h1>No Images Found</h1>';
        }
    } catch (error) {
        console.error('Error fetching image:', error);
        container.style.backgroundColor = 'lightgray';
        container.innerHTML = '<h1>Image Load Error</h1>';
    }
}
...
searchInput.addEventListener('change', (event) => {
    ...
    updateBackgroundImages(cityName);
});

updateBackgroundImages('new york');
